@php
    ...
    $field['wrapper']['data-init-function'] = 'bpFieldInitBrowseMultipleElementCustom';
...
<div class="list" data-field-name="{{ $field['name'] }}">
    @if ($multiple)
        <input type="hidden" class="file" data-marker="multipleBrowseInput" name="{{ $field['name'] }}" value="{{ json_encode($value) }}">
    @else
...
<script type="text/html" data-marker="browse_multiple_template">
    <div class="input-group input-group-sm">
        <input type="text" class="form-control file" @include('crud::fields.inc.attributes') readonly>
        <input type="text" class="form-control alt" placeholder="Alt">
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            function bpFieldInitBrowseMultipleElementCustom(element) {
                ...
                if ($input.val() != '' && $input.val() != null && $multiple === 'true') {
                    ...
                    if (Array.isArray($paths) && $paths.length) {
                        ...
                        $paths.forEach(function (item) {
                            ...
                            newInput.find('.file').val(item.paths);
                            newInput.find('.title').val(item.title);
                            newInput.find('.alt').val(item.alt);
                ...
                element.on('saveToJson', function(event) {
                    let $paths = element.find('.file').not('[type=hidden]').map(function (idx, item) {
                        let obj = $(item);
                        let object = {};
                        object.paths = obj.val();
                        object.alt = obj.next().val();
                        object.title = obj.next().next().val();
                        return object;
                    }).toArray();
                ...
                if ($multiple === 'true') {
                    ...
                    element.on('createInputsForItemsSelectedWithElfinder', element, function(event, files) {
                        files.forEach(function (file) {
                            var newInput = $($template);
                            newInput.find('.file').val(file.path);
                            $list.append(newInput);
                        });
