...
<?php
    $options = [];
    if(!empty($entry)) {
        $type = \App\ItemKey::getTypeSection($entry);
        $keys = \App\ItemKey::where('item_id',$entry->id)->where('type',$type)->pluck('key_id','key_id')->toArray() ?? [];
        $keyName = \App\Models\Keys::whereIn('id',$keys)->pluck('name','id')->toArray();
    }
?>
@php
...
foreach ($field['options'] as $option) {
    if(isset($keyName[$option->id])) {
        $options[$option->getKey()]['selected'] = true;
        $options[$option->getKey()]['value'] = $option->{ $field['attribute']};
    } else {
        $options[$option->getKey()]['selected'] = false;
        $options[$option->getKey()]['value'] = $option->{ $field['attribute']};
    }
}
?>
<select
    ...
    @if (isset($field['model']))
        @foreach($options as $key => $option)
            <option value="{{$key}}" {{$option['selected'] ? 'selected' : null}}>{{ $option['value'] }}</option>
        @endforeach
        {{--@foreach ($field['options'] as $option)
